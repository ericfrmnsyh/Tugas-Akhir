<?php
    ...
    if(isset($_POST['add'])){
		function checkUser($username)
		{
			...
			$query = $dbc->prepare("SELECT * FROM user WHERE user_username = :username");
    ...
    if (empty($username_error) && empty($email_error) && empty($hp_error) && empty($pass_error) && empty($confirm_error)) {
        $state = $dbc->prepare("INSERT INTO user (user_username,user_password,user_phone,user_email) VALUES (:username,SHA2(:pass,0),:phone,:email)");
